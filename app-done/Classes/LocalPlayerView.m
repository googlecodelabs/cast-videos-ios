...
- (void)loadMediaImage {
  ...
  dispatch_async(queue, ^{
    UIImage *image =
        [UIImage imageWithData:[SimpleImageFetcher
                                   getDataFromImageURL:self.media.imageURL]];
...
- (void)handleMediaPlayerReady {
  ...
  if (!self.streamDuration) {
    ...
    self.totalTime.text = [self updateTimersForTime:self.streamDuration];
  }
...
- (void)notifyStreamPositionChanged:(CMTime)time {
  ...
  self.totalTime.text = [self updateTimersForTime:remainingTime];
}
...
- (NSString *)updateTimersForTime:(NSTimeInterval)timeInterval {
  NSString *string = nil;
  NSInteger seconds = (NSInteger)round(fabs(timeInterval));
  ...
  if (hours > 0) {
    string = [NSString stringWithFormat:@"%d:%02d:%02d", (int)hours,
                                        (int)minutes, (int)seconds];
  } else {
    string = [NSString stringWithFormat:@"%d:%02d", (int)minutes, (int)seconds];
  }
  return string;
}
