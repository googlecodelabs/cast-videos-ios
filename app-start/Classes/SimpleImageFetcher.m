...
+ (UIImage *)scaleImage:(UIImage *)image toSize:(CGSize)newSize {
  ...
  CGRect scaledImageRect =
      CGRectMake(0.0, 0.0, scaledSize.width, scaledSize.height);
...
+ (NSData *)getDataFromImageURL:(NSURL *)urlToFetch {
  ...
  NSString *cacheFileName =
      [self sha1HashForString:[urlToFetch absoluteString]];
  NSURL *cacheDirectory = [[[fileManager URLsForDirectory:NSCachesDirectory
                                                inDomains:NSUserDomainMask]
      lastObject] URLByAppendingPathComponent:@"thumbnails"];
  NSURL *cacheFileURL =
      [cacheDirectory URLByAppendingPathComponent:cacheFileName];
    ...
    [fileManager createDirectoryAtURL:cacheDirectory
          withIntermediateDirectories:YES
                           attributes:nil
                                error:&error];
    if (error) {
      NSLog(@"Received an error trying to create a directory %@",
  ...
  if (error) {
    NSLog(@"Received an error trying to save a cached file %@",
          [error localizedDescription]);
  }
  return imageData;
}
